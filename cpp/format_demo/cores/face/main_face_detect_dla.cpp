    ...
    try {
        ...
        if (!output_detection_txt.empty()) {
            ...
            if (detection_txt_file.is_open()) {
                detection_txt_file
                        << "filename,bbox_x,bbox_y,bbox_width,bbox_height,score,kps0_x,kps0_y"
                        << ",kps1_x,kps1_y,kps2_x,kps2_y,kps3_x,kps3_y,kps4_x,kps4_y\n";
        ...
        for (const auto &entry_path: image_paths) {
            ...
            if (ext == ".jpg" || ext == ".png" || ext == ".jpeg") {
                ...
                for (int i = 0; i < _n; i++) {
                    auto faces = face_app.get(gpu_img);
                }
        ...
        if (detection_txt_file.is_open()) detection_txt_file.close();
        if (embedding_txt_file.is_open()) embedding_txt_file.close();
        ...
        for (size_t i = 0; i < paths_list.size(); ++i) {
            if (keep_mask[i]) { kept_paths.push_back(paths_list[i]); }
            else { removed_paths.push_back(paths_list[i]); }
        }
        ...
        if (!removed_paths.empty()) {
            ...
            for (const auto &name: removed_paths) std::cout << " - " << name << std::endl;
        }
