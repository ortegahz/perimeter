    ...
    try {
        ...
        while (true) {
            ...
            try {
                ...
                // 【修改】直接配置越界规则的规格，包括无序点、产品方向和灵敏度等级。
                // Processor 内部将负责转换这些规格。
                // 示例1: 一条竖线, AB方向报警, 灵敏度等级为1 (最高灵敏度 -> 最小触发面积)
                proc_config.line_rules_spec[CAM_ID]["Line_1"] = {
                        {1400, 1280}, {1200, 60}, "any", -1
                };

                // 示例2: 一条横线, BA方向报警, 灵敏度等级为10 (最低灵敏度 -> 最大触发面积)
                proc_config.line_rules_spec[CAM_ID]["Line_2"] = {
                        {2000, 700}, {1000, 700}, "BA", 10
                };
